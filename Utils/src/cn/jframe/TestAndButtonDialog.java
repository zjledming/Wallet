 ...
 * 
 */
public class TestAndButtonDialog extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TestAndButtonDialog frame = new TestAndButtonDialog();
	...
	public TestAndButtonDialog() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ((TestAndButtonDialog.this.pwd.getText().length() > 0)
					...
					// 提示框
					JOptionPane.showMessageDialog(TestAndButtonDialog.this,
							"用户名和密码不能为空！");
				}
			}
		});
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (TestAndButtonDialog.this.pwd1.getText().length() == 0)
					JOptionPane.showMessageDialog(TestAndButtonDialog.this,
							"不是密文，解密出错！");
			}
		});
